...
export async function createSqlJsAdapter(filePath) {
  ...
  // Flush on shutdown (use once to prevent listener accumulation)
  ...
  return {
    ...
    async get(sql, params = []) { return get(sql, params); },
    async all(sql, params = []) { return all(sql, params); },
    async exec(sql) { return exec(sql); },
    async transaction(fn) { return transaction(fn); },
    close,
    raw: db
  };
}
